...
steal('jquery/class', 'jquery/lang/string', function() {
	 ...
	 */
	{
		...
		save: function( success, error ) {
			if(this.type == 'ImageItem') {
				if($('#height').next('span').length > 0)
					$('#height').next('span').remove();
				else{
					if(!Number(this.width) && !Number(this.height)) {
						$('#height').after('<span class="error">Please enter at least one value (height or width).</span>');
						$('#width').val(240);
						$('#height').val(320);
						this.width = 240;
						this.height = 320;
